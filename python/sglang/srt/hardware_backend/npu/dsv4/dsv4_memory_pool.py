...
class NPUDeepSeekV4SingleKVPool(DeepSeekV4SingleKVPool):
    ...
    ``npu_kv_quant_sparse_attn_sharedkv`` reads KV in PA_ND layout
    ``(num_pages, kernel_page_size, num_kv_heads=1, dim)`` and requires ``dim``
    to be the A5 packed KV width (640): rope bf16 bytes, quantized nope fp8,
    fp8 scale bytes, then padding. So the c4/c128 pools (whose token-level
    page_size is ``page_size // ratio``) are allocated at the GLOBAL
    ``kernel_page_size`` rather than their own per-ratio page_size; the SWA pool
    uses ``kernel_page_size == page_size``. The CUDA fp8-packed-bytes layout
    (the base ``create_buffer``) is untouched.
    """
    ...
    def a5_packed_kv_dim(self) -> int:
        ...
        scale_dim = math.ceil(nope_dim / self.quantize_block_size)
...
class DSV4NPUTokenToKVPool(DeepSeekV4TokenToKVPool):
    ...
    def set_swa_buffer(
        ...
        ``cache`` shape: (num_tokens, num_kv_heads=1, dim) or (num_tokens, dim).
        On A5 the buffer is the packed fp8 cache consumed by
        ``npu_kv_quant_sparse_attn_sharedkv``; ``kv_compress_epilog`` converts
        the incoming bf16 ``[T, 512]`` rows and writes them by flat slot id.
        """
